...
export const CareersManager = () => {
  ...
  // Para faculty-admin NO usamos filtros porque el backend ya filtra automáticamente
  const [filterUniversityId, setFilterUniversityId] = useState(isFacultyAdmin ? '' : (userUniversityId || ''));
  const [filterFacultyId, setFilterFacultyId] = useState(isFacultyAdmin ? '' : (userFacultyId || ''));
  ...
  // Actualizar filtros cuando user data está disponible (solo para non-faculty-admin)
  useEffect(() => {
    if (isFacultyAdmin) return; // Faculty-admin no usa filtros de UI
  ...
  }, [userUniversityId, userFacultyId, isFacultyAdmin]);
  ...
  const handleCreate = () => {
    ...
    setFormData({
      career_id: '',
      name: '',
      faculty_id: userFacultyId || '', // Pre-llenar para faculty-admin
      university_id: userUniversityId || '' // Pre-llenar para university-admin y faculty-admin
    });
  ...
  // Filtrar carreras (solo para non-faculty-admin, ya que faculty-admin recibe data pre-filtrada del backend)
  const filteredCareers = isFacultyAdmin ? careers : careers.filter(career => {
  ...
  const getTitle = () => {
    if (isFacultyAdmin && userFacultyId && userUniversityId) {
      ...
      return `Gestión de Carreras - ${faculty?.name || userFacultyId} de ${university?.name || userUniversityId}`;
    }
    if (userUniversityId && !isSuperAdmin) {
      const university = universities.find(u => u.university_id === userUniversityId);
      return `Gestión de Carreras - ${university?.name || userUniversityId}`;
    }
